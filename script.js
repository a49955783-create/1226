...
const opsName=document.getElementById("opsName"),opsCode=document.getElementById("opsCode"),
deputyName=document.getElementById("deputyName"),deputyCode=document.getElementById("deputyCode"),
dataTable=document.getElementById("dataTable").querySelector("tbody"),finalOutput=document.getElementById("finalOutput"),
...
if(!opsName.value.trim()||!deputyName.value.trim()){finalOutput.innerHTML="<p style='color:red'>⚠️ الرجاء إدخال اسم العمليات والنائب</p>";return}
let result=`<div class='section-box'><b>📌 استلام العمليات 📌</b></div>`;
result+=`<div class='section-box'>اسم العمليات : ${opsName.value} ${opsCode.value}</div>`;
result+=`<div class='section-box'>النائب : ${deputyName.value} ${deputyCode.value}</div>`;
...
if(outService.length){outService.forEach(r=>result+=buildLine(r)+"<br>")}else{result+="(0)<br>"}
result+=`</div>`;
...
opsName.addEventListener("input",updateFinalOutput);opsCode.addEventListener("input",updateFinalOutput);
deputyName.addEventListener("input",updateFinalOutput);deputyCode.addEventListener("input",updateFinalOutput);
